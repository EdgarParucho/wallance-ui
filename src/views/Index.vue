...
function onSubmit() {
  getAccessTokenWithPopup()
    .then((accessToken) => {
      if (!user.value.email_verified) return showAlert({
        type: "info",
        text: "Please follow the verification link that we've sent to the provided email, then log in.",
      });
...
function getUserData(data) {
  ...
  authStore.login(data)
    .then((message) => {
      authStore.accessToken = token.value;
