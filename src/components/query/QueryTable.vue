<template>
  <div class="my-4 w-11/12 xl:w-2/3 mx-auto">
    ...
    <table class="w-full mx-auto text-sm shadow-md">
      <thead v-if="!underLgBreakpoint">
        <tr class="p-1 bg-stone-100 dark:bg-stone-700">
          <th v-for="header in headers" :key="header" class="border border-white dark:border-stone-900">
            {{ header }}
          </th>
        </tr>
      </thead>
      <tbody>
        ...
        >
          <td class="xl:border border-white dark:border-stone-900 h-8 xl:w-40 lg:px-2 text-end">
            {{ formattedDate(record) }}
          </td>
          <td class="xl:border border-white dark:border-stone-900 h-8 lg:w-48 lg:px-2">
            <div class="flex items-center">
              <span>{{ getFundName(record.fundID) }}</span>
              <span v-if="record.type === 0">, {{ getFundName(record.otherFundID) }}</span>
            </div>
          </td>
          <td class="xl:border border-white dark:border-stone-900 px-2 h-8 2xl:min-w-96">
          ...
          <td class="xl:border border-white dark:border-stone-900 lg:w-40 lg:px-2 h-8">
          ...
          <td class="xl:border border-white dark:border-stone-900 lg:w-24 h-8">
